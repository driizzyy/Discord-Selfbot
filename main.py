...
def check_for_updates():
    ...
    repo_user = "driizzyy"  # Change to your GitHub username
    repo_name = "Discord-Selfbot"  # Change to your repo name
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Look for __version__ string in remote code
            import re
...
async def help(ctx):
    ...
    help_text = f"""
**Driizzyy SelfBot | Prefix: `{prefix}`**\n
**Commands:**\n
> :space_invader: `{prefix}driizzyy` - Show my social networks.
...
> :zzz: `{prefix}weather <city>` - Checks the Weather in a specified city without an API key.
...
async def github(ctx, username: str = None):
    await ctx.message.delete()
    if not username:
        await ctx.send("> **[ERROR]**: Provide a GitHub username.\n> __Command__: `github <username>`", delete_after=5)
        return
    ...
    if response.status_code != 200:
        await ctx.send(f"> **[ERROR]**: GitHub user `{username}` not found.", delete_after=5)
        return

    data = response.json()

    embed = discord.Embed(
        title=f"GitHub: {data['login']}",
        url=data['html_url'],
    ...
    embed.set_footer(text=f"Account created: {data['created_at'][:10]}")
...
async def userinfo(ctx, user: discord.Member = None):
    ...
    joined_at = user.joined_at.strftime('%d-%m-%Y %H:%M:%S') if user.joined_at else "N/A"
    await ctx.send(
        f"> **User Information**\n"
        f"Username: `{user}`\n"
        f"ID: `{user.id}`\n"
...
async def weather(ctx, *, city: str = None):
    await ctx.message.delete()
    if not city:
        await ctx.send("> **[ERROR]**: Please provide a city.\n> __Command__: `weather <city>`", delete_after=5)
        return
    weather_url = f"https://wttr.in/{city.replace(' ', '+')}?format=3"
    try:
        response = requests.get(weather_url)
        await ctx.send(f"> 🌤️ Weather for **{city.title()}**: `{response.text.strip()}`")
    except:
        await ctx.send("> **[ERROR]**: Could not fetch weather data.", delete_after=5)
        
@bot.command()
async def choose(ctx, *, options: str = None):
    await ctx.message.delete()
    if not options:
        await ctx.send("> **[ERROR]**: Please provide options separated by commas.\n> __Command__: `choose option1, option2, option3`", delete_after=5)
        return
...
async def tokeninfo(ctx, usertoken: str=None):
    ...
    if res.status_code == 200:
        ...
        > :paperclip: __Other__\n\tLocale: `{locale} ({language})`\n\tEmail Verified: `{verified}`"""
            ...
            await ctx.send(embed, file=discord.File("img/driizzyy.gif"))
        except Exception as e:
